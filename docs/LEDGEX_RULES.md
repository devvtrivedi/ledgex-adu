...
>
> `build/ledgex_source.py` owns `INVARIANTS` and `MAKE_TARGETS`. `build_spec_v1_16.py` imports the same objects. A change that produces different invariant text in the two artifacts fails document QA (`python build/qa_check.py`).
...
*Generated 2026-08-07 by `build/build_rules_v1_4.py`. Source of record: `build/ledgex_source.py`.*