...
| Rank | Document | Role |
|---|---|---|
| 1 | Engineering Reference Spec v1.12 | Machine-executed build contract. |
...
>
> `build/ledgex_source.py` owns `INVARIANTS` and `MAKE_TARGETS`. `build_spec_v1_12.py` imports the same objects. A change that produces different invariant text in the two artifacts fails document QA (`python build/qa_check.py`).
...
Engineering Reference Spec v1.12 - executable contract
