...
| 25 | The disposable-database enforcement question — `make db-test`'s bare default points at `ledgex_schema_check` | **Still open, report only** — P17 | P14's deferral reasoning ("changing the default affects every other target") tested, not inherited: a second Makefile variable read only by `db-test`'s own recipe leaves `schema`/`schema-dump`/`migrate`/`migrate-verify` completely untouched — mechanically confirmed, not assumed. Three options argued: (a) a separate default for `db-test` only (e.g. `ledgex_test`, matching CI's fresh-database-per-run shape) — cheapest, but a second variable name to learn, a database that doesn't exist on a fresh clone (fails loud instead of silently, still a real first-run cost), and CI's own explicit `DATABASE_URL=` override would need updating in the same change; (b) refuse without an explicit opt-in flag — cheapest, but the weakest real protection, exactly the ritual `db/README.md`'s own three-way `schema`/`migrate`/`migrate-baseline` decision procedure already argues against; (c) refuse unless the database carries an actual disposability marker — strongest (verifies something true about the database, not a name or a flag, matching this repo's own constraint-over-convention preference), highest cost (the marker mechanism doesn't exist yet — P14's own report already named this as needing its own pass, still true). Recommendation recorded, not built: (a) now, (c) named explicitly as the stronger deferred fix, its own future package; (b) not recommended. |
