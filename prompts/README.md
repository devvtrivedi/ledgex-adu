...
| P16 | [Finding #18: exceptions stranded by a detector_version bump](P16-detector-version-retirement.md) | done, pushed | TBD |
...
| 24 | `ledgex_schema_check` carries removable class-3 and permanent class-2 fixture rows from repeated `db/tests/invariants.sql` runs | **Removable share not run — needs an explicit ask** — P14 | Full before-state queried, not sampled: `parcel` 40, `fact` 324 (class 2, permanent — `fact_no_delete`/0017 + the FK, no removal path short of a rebuild) and `parcel_exception` 40, `property_file` 20, `property_file_fact` 8, `job_run` (`test.%` job_key) 8, `exception_evidence` 0, `source_feature_identity` 0 (class 3, 76 rows total, removable today with no schema conflict — confirmed by direct `DELETE` in P14's own investigation). P14 built teardown so future runs stop adding to the class-3 share, and stated class 2's permanence as a loud precondition, but neither retroactively touches what's already sitting in this database. Proposed as its own future step in [P14-invariants-sql-teardown.md](P14-invariants-sql-teardown.md) §6 — not run. |
| 25 | The disposable-database enforcement question — `make db-test`'s bare default points at `ledgex_schema_check` | **Named, deliberately not decided** — P14 | `DATABASE_URL ?= postgresql://localhost/ledgex_schema_check` (`Makefile`) is what let a `make db-test` with no override contaminate the project's own shared local dev database in the first place (see #24). P14 named three candidate responses (selective teardown, namespaced-and-documented accumulation, refuse-to-run-against-a-non-disposable-database) and built the first two; the third — making the suite refuse to run, or at minimum refuse silently, against a database not marked disposable — was deliberately left undecided: changing `DATABASE_URL`'s default is a behaviour change affecting every other `make` target, not scoped to `db-test` alone, and *how* a database earns the "disposable" label (a naming convention, an explicit marker table, something else) is its own design question this repo's own `db/README.md` three-way `make schema`/`migrate`/`migrate-baseline` precedent argues against guessing at. Own package, later. |
| 26 | Teardown does not run when `ON_ERROR_STOP` aborts the suite — a failing run still accumulates class-3 rows | **Named in `invariants.sql`'s own comment, not in this table until now** — P14 | `db/tests/invariants.sql:4637`'s own teardown-block comment: "if that check fails, `ON_ERROR_STOP` aborts the script right there and teardown never runs, leaving the failing run's data in place to look at." Deliberate, not an oversight — the alternative (teardown running regardless of pass/fail) would erase the evidence a failing run exists to produce. The real consequence, not previously written down anywhere the findings table would surface it: a failing run is exactly the run someone is most likely to retry immediately, and each retry that also fails adds another full set of class-3 rows before anyone gets a passing run to clean them up. Bounded by class 2's own growth rate in practice (one parcel/its facts per attempt regardless of pass or fail), but not addressed by P14's teardown, which only ever runs on the success path by design. |
| 27 | `ledgex_schema_check`'s `schema_migrations` ledger had drifted six migrations behind before P6, and had drifted two migrations behind again (topped out at `0047`, missing `0048`/`0049`) by the start of P16 | **Closed** — P16 | Second occurrence, not the first — `db/README.md`'s own decision-procedure section already names the pre-P6 six-migration drift as the reason that procedure exists at all. This time: `0048`/`0049` (P10) landed against fresh scratch databases and CI, never against this shared local one, so its `parcel_exception` was still carrying the pre-`0049` `parcel_exception_one_open_per_detector_reason` index (not the `_coalesced` one) — confirmed live before fixing, not assumed — right up until P16 queried it for finding #18. Fixed per `db/README.md`'s own "existing database with a `schema_migrations` table already" case: `make migrate` (`applying 0048_refusals_codes_valid_reject_null_shapes.sql`, `applying 0049_parcel_exception_reason_coalesced.sql`, `applied 2 migration(s)`), then `make migrate-verify` (`MATCH -- ledgex_schema_check's live schema is exactly what its ledger claims. 49 migration(s) verified.`). No general fix proposed here — `make migrate`/`migrate-verify` already exist and did their job the moment someone ran them; the gap is that nothing runs them automatically against a shared local database between sessions, which is the same unresolved shape as finding #25's disposable-database question, not re-litigated here. |
