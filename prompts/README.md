...
| 13 | Detector reruns create duplicate open exceptions | **Closed** — `8a7286e`/`4d0f7ea` (P5), migration `0045` | Partial unique index `(parcel_id, detector_key, detector_version, detail->>'reason') WHERE outcome='open'`, plus `load_zoning` checks `existing_open` before writing (`scripts/ingest_zoning_permits.py:635-680`). **Not the same problem as P8** ([P8-exception-resolution-undefined.md](P8-exception-resolution-undefined.md)): 0045 stops a *repeat* of an unchanged condition; it does nothing when the condition *changes* — that's P8, still an open, unfixed finding in its own right. |
