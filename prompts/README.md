...
| P22 | [Finding #27 reopened, then core/model.Fact adopted at the loaders](P22-fact-adoption.md) | pending | — |
...
| 29 | `docs/LEDGEX_SPEC.md` §2 contradicts itself on whether `core/` may import `infra/` — "core/* may import core/model and stdlib/third-party only" in one bullet, "Any of core/, commerce/, jurisdictions/, pipelines/, api/ may import infra/" two bullets later — and `.importlinter` enforces neither direction | **Open, reported not fixed — P22** | Found while settling `Fact.value`'s encoding contract: option (a) (native Python values, `insert_facts()` does the `json.dumps()`) would have needed `infra.values.decimal_default` for geometry's `Decimal` fields inside `core/store.py`, which is exactly the import §2's first bullet forbids and its later bullet allows. Not resolved here — option (a) was not chosen (see design decision (c) in `core/model.py`'s own module docstring for the option that WAS chosen and why it sidesteps this entirely), so nothing in this package actually needs the answer, but the contradiction is real and independent of that choice. Whoever next needs `core/` to import `infra/` for a forced reason (not this package) has to resolve it with a real spec amendment and a §12 row, not a judgement call in a commit message — flagged here so it isn't rediscovered from scratch. |
| 30 | `scripts/run_p5_acceptance.sh` is not safe to rerun twice against the same already-populated database — `scripts/check_p5_acceptance.py:220-224`'s `check_permits_after_b` asserts parcel `23712112`'s `permits.active` fact is a brand-NEW fact (`supersedes_fact_id IS NULL`) after phase B, which is only true the first time the script's own A1/A2/B sequence runs against a given database; a second full run finds a live fact already there from the first run and correctly supersedes it instead, failing an assertion that encodes "first run" as an unstated precondition | **Open, reported not fixed — P22** | Found while verifying the `Fact` adoption end-to-end, not caused by it: isolated by reverting to the pre-P22 `core/store.py`/`scripts/ingest_*.py` (`git diff > patch`, `git checkout --`, confirmed via `grep` that the isinstance guard this package adds was genuinely absent) and rerunning the identical two-invocations-same-database sequence against a fresh scratch database — the same `[FAIL] after B: 23712112 permits.active = true, NEW fact (no supersession)` occurred on unmodified code. This package's own suite verification instead used three independent fresh migrations-only databases (one run each) rather than one database run twice, which is what the script's own header comment says the "twice" half of CONVENTIONS.md's suite discipline means for it — not re-litigated or fixed here; the header comment and the checker's own hardcoded first-run assumption are the two places a future fix would need to touch. |
