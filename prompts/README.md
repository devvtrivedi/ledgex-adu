...
| 33 | `make conformance` is wired into `db.yml` as a real CI gate but has never been demonstrated to fail — CONVENTIONS' own hard rule ("every check must be seen to fail at least once... on the real runner") is genuinely unsatisfied for it | **Open** | Found during P29's audit of P26's own skipped close-out: `git log` carries exactly one P26 commit (`3980560`) — the deliberate break-then-revert P26 section 4 originally described in the past tense never actually happened; that section now says so honestly instead of leaving the false claim stand. Only evidence that exists: a local RED proof (a field_key absent from `field_definition` planted into `sources.yaml`, confirmed the two predicted checks failed, restored) and the main build's own CI outcome — neither is the same as watching the CI-wired gate itself go red on the runner it actually guards. This is the same silently-passing-gate class `docs.yml`'s own day-long red window and finding #27's drifting ledger both belong to: a gate that has only ever been observed green is, per CONVENTIONS' own words, "a comment." Cross-linked in [P26-jurisdictions-pack-format.md](P26-jurisdictions-pack-format.md) section 4. |
