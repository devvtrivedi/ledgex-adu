...
| 35 | Bulletin #210's own page 3: San José's City and State ADU Development Standards give materially different answers for the same conclusion (detached ADU max height: 25 ft under City standards, 18 ft under State) and an applicant must elect one — nothing in this schema records that election | **Open, its own future package** | Found from a primary source (the bulletin itself, not a hypothetical), not a code defect this package can fix: *"YOU HAVE A CHOICE: Design the ADU following either City Standards (Municipal Code 20.80.175) or State Standards (Municipal Code 20.80.176). The standards cannot be mixed; you must choose either all City standards or all State standards."* §7.4's own conclusion model (`jurisdictions/ca_san_jose/conclusions.yaml`, still undesigned) keys a rule lookup on jurisdiction + as-of alone — no `property_file`, `parcel`, or request-scoped input anywhere in this schema records which regime an applicant elected. A rule keyed this way cannot, even in principle, answer "what is the max height for this ADU" — only "what is the max height under the City regime," which this package's own `rule_key` (`adu.detached.max_height.city_standards`) states explicitly, so it can never be silently read as the universal answer. Left open, its own package: this is a real gap in the conclusion model itself, not something P31's one hardcoded rule_key can paper over. |
| 36 | The one real `rule` row had two independent `ON CONFLICT (id) DO NOTHING` seeders — `db/seeds/day4_sources.sql` and `scripts/check_golden.py` — finding #32's exact shape, reintroduced by P31 one package after #32 was fixed for `source` | **Closed — P32** | Byte-identical today, nothing enforced that; a citation/`pack_version` edit to one seeder alone would pass `make golden` silently (`ruleset_version` is only `rule_key@version`). Fixed at the source: `check_golden.py`'s `INSERT` now ends `ON CONFLICT (id) DO UPDATE SET` naming all fourteen non-key columns, same remedy P26 used for #32. Verified the trigger interaction before relying on it, not on faith: `0013`'s `rule_no_destructive_update()` does not fire on an identical-value `DO UPDATE` (both guards short-circuit on `IS NOT DISTINCT FROM`) — confirmed directly against the real row, `INSERT 0 1`, exit 0, no exception. Then proved the other half for real: one column deliberately drifted, immediate `ERROR: I18 violated: rule ... is immutable. Only effective_to may be set (NULL -> a date, once). A correction is a new rule row at version + 1, never an UPDATE.` **Stronger than #32's own remedy, not merely the same fix repeated**: `source` has no immutability trigger, so `DO UPDATE` there only removes the ordering race. `rule` does — `DO UPDATE` here makes any *future* drift between the two seeders impossible to land silently at all; the next `make golden` run after any edit raises `I18 violated` by name instead of quietly picking a winner. Full evidence in [P32-dual-seeder-and-rule-election-scope.md](P32-dual-seeder-and-rule-election-scope.md) section 1. |
| 37 | `make golden` performs an irreversible write — `rule_no_delete` blocks removal unconditionally, so every database `make golden` is run against now permanently carries the P31 rule row | **Open — reported, not fixed in this package** | Finding #28's exact class (a routine check performing an irreversible write), found this time by reading the trigger rather than discovering it on a WORM bucket after the fact. Checked per-database, not assumed: `ledgex_schema_check` — `make migrate-verify` run first (51 migrations, `MATCH`), then queried directly: `SELECT count(*) FROM rule WHERE id='ca_san_jose.adu_detached_max_height_city_standards.v1'` → `0`. Does **not** currently carry the row. `ledgex_test` does not exist locally at all. The real, live risk: `Makefile`'s own `DATABASE_URL` default is `postgresql://localhost/ledgex_schema_check` — a bare `make golden` with no override would silently, permanently plant this row into the shared dev database on the very next local run. CI's own ephemeral `ledgex_ci`/`ledgex_ci_p5`/`ledgex_ci_phaseb` databases are not a persistent concern by the same logic that makes them safe everywhere else in this project — destroyed with the runner at the end of every job regardless of what the trigger blocks. **Whether `make golden` should be doing this at all — argued, not decided**: every other row `check_golden.py`/`check_conformance.py` self-seed (`licence`, `jurisdiction`, `source`, `field_definition`) is either trigger-free or already idempotency-safe by construction; `rule` is categorically different post-`0013` — irreversible, not merely re-correctable. A "check" permanently marking every database it touches is architecturally surprising for something whose job is verification, not seeding. Two real alternatives, neither built here: (a) `make golden` fails loud with an actionable message when the rule row is absent, requiring `db/seeds/day4_sources.sql` (the real seed layer) to have run first — correct in spirit, but reopens the "CI never runs `db/seeds/`" gap this project already worked around once for exactly this class of row; (b) leave `check_golden.py` as the seeder but treat it explicitly as a one-time, irreversible bootstrap step, documented as such, not silently bundled inside a target named `check`. No change made in this package — reported for a decision, per instruction. |
